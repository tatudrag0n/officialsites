...
公開リポジトリ `officialsites` にはRuleset `Mifron main PR gate` を設定済みです。`main` はPull Request経由のみ、強制push禁止、レビュー会話の解決必須、バイパスなしです。非公開のMinoru/MifronPluginは現在のGitHubプランではRuleset API対象外のため、別レビュアーを追加してからブランチ保護を設定します。
